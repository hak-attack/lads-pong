...
interface AddLossFormProps {
  playerId: string
  onSuccess?: () => void
  onCancel?: () => void
}

export function AddLossForm({ playerId, onSuccess, onCancel }: AddLossFormProps) {
  ...
  const handleSubmit = async () => {
    ...
    if (isNaN(setsWonByLoserNum) || setsWonByLoserNum < 0) {
      toast({
        title: 'Invalid sets',
        description: 'Please enter valid number of sets won by you',
        variant: 'destructive',
      })
      return
    }
    ...
    if (numberOfSetsNum === 1) {
      if (setsWonByLoserNum !== 0) {
        toast({
          title: 'Invalid sets',
          description: 'In a 1 set match, you must have 0 sets',
          variant: 'destructive',
        })
        return
      }
      ...
      if (setsWonByLoserNum > maxLoserSets) {
        toast({
          title: 'Invalid sets',
          description: `You cannot have won more than ${maxLoserSets} sets in a best of ${numberOfSetsNum} match`,
          variant: 'destructive',
        })
        return
      }
    ...
    try {
      ...
      toast({
        title: 'Loss recorded',
        description: `${selectedOpponent?.name} defeated ${player?.name} ${setsWonByWinnerNum}-${setsWonByLoserNum} (last set: ${winnerScoreNum}-${loserScoreNum})`,
      })
      setOpponentId('')
      setNumberOfSets('3')
      setScorePerSet('11')
  ...
  return (
    <div className="space-y-4">
      <div>
        <h2 className="text-xl font-bold mb-2">Add Loss</h2>
